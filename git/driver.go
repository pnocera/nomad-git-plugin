...
var (
	// PluginID is the gitexec plugin metadata registered in the plugin
	// catalog.
	...
	// PluginConfig is the gitexec factory function registered in the
	// plugin catalog.
	PluginConfig = &loader.InternalPluginConfig{
		Config:  map[string]interface{}{},
		Factory: func(ctx context.Context, l hclog.Logger) interface{} { return NewGitExecDriver(ctx, l) },
	}

	errDisabledDriver = fmt.Errorf("git_exec is disabled")
)

// PluginLoader maps pre-0.9 client driver options to post-0.9 plugin options.
func PluginLoader(opts map[string]string) (map[string]interface{}, error) {
	conf := map[string]interface{}{}
	if v, err := strconv.ParseBool(opts["driver.git_exec.enable"]); err == nil {
		conf["enabled"] = v
	}
	if v, err := strconv.ParseBool(opts["driver.git_exec.no_cgroups"]); err == nil {
		conf["no_cgroups"] = v
	}
	return conf, nil
}
...
type Config struct {
	...
	// Enabled is set to true to enable the git_exec driver
...
// NewGitExecDriver returns a new DriverPlugin implementation
func NewGitExecDriver(ctx context.Context, logger hclog.Logger) drivers.DriverPlugin {
...
func (d *Driver) buildFingerprint() *drivers.Fingerprint {
	...
	if d.config.Enabled {
		...
		attrs["driver.git_exec"] = pstructs.NewBoolAttribute(true)
	} else {
